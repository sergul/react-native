...
export const Timer = () => {
  ...
  const callback = useCallback((currentMilliseconds: number) => {
    ...
    const resetToZero = (value: number) => (value > 59 ? 0 : value);
    const seconds = resetToZero(Math.floor(converted));
    const milliseconds = Math.floor((converted - seconds) * 100);
    const minutes = resetToZero(Math.floor(seconds / 60));
  ...
  const hoursStr = prependZero(timePassed.hours);
  const minutesStr = prependZero(timePassed.minutes);
  const secondsStr = prependZero(timePassed.seconds);
